...
func (o *OTLPReceiver) processRequest(protocol string, header http.Header, in *otlppb.ExportTraceServiceRequest) {
	for _, rspans := range in.ResourceSpans {
		...
		rtags := make(map[string]string, len(rspans.Resource.Attributes))
		for _, attr := range rspans.Resource.Attributes {
			rtags[attr.Key] = anyValueString(attr.Value)
		}
		lang := rtags["telemetry.sdk.language"]
		if lang == "" {
		...
		tags := info.Tags{
			...
			TracerVersion:   fmt.Sprintf("otlp-%s", rtags["telemetry.sdk.version"]),
		...
		for _, libspans := range rspans.InstrumentationLibrarySpans {
			...
			for _, span := range libspans.Spans {
				...
				tracesByID[traceID] = append(tracesByID[traceID], convertSpan(rtags, lib, span))
			}
		}
		p := Payload{
...
func convertSpan(rtags map[string]string, lib *otlppb.InstrumentationLibrary, in *otlppb.Span) *pb.Span {
	...
	span := &pb.Span{
		...
		Service:  rtags["service.name"],
		Resource: in.Name,
		Meta:     rtags,
		Metrics:  map[string]float64{},
	}
	// auto-keep all incoming traces
	span.Metrics[sampler.KeySamplingPriority] = float64(sampler.PriorityAutoKeep)
	if _, ok := span.Meta["env"]; !ok {
		if env := span.Meta["deployment.environment"]; env != "" {
			span.Meta["env"] = env
		} else if env := rtags["env"]; env != "" {
			span.Meta["env"] = env
		}
	}
	if _, ok := span.Meta["version"]; !ok {
		if ver := rtags["service.version"]; ver != "" {
...
func byteArrayToUint64(b []byte) uint64 {
	out, err := binary.ReadUvarint(bytes.NewReader(b))
	if err != nil {
		return 0
	}
	return out
...
func anyValueString(a *otlppb.AnyValue) string {
	...
	case *otlppb.AnyValue_DoubleValue:
		return strconv.FormatFloat(v.DoubleValue, 'e', 2, 64)
